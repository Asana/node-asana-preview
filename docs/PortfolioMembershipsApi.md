# AsanaPreview.PortfolioMembershipsApi
...
> GetPortfolioMembership200Response getPortfolioMembership(portfolioMembershipGid, opts)
...
let portfolioMembershipGid = 1331; // String | 
let opts = {
  'optPretty': true, // Boolean | Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.
  'optFields': ["user","portfolio"] // [String] | This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.
};

apiInstance.getPortfolioMembership(portfolioMembershipGid, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> GetPortfolioMemberships200Response getPortfolioMemberships(opts)
...
> GetPortfolioMemberships200Response getPortfolioMembershipsForPortfolio(portfolioGid, opts)
...
apiInstance.getPortfolioMembershipsForPortfolio(portfolioGid, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' +  JSON.stringify(data, 0, 2));
