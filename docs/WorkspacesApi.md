...
# **addUserForWorkspace**
> UserBaseResponseData addUserForWorkspace(body, workspace_gid, opts)
...
> EmptyResponseData removeUserForWorkspace(body, workspace_gid)
...
let apiInstance = new AsanaPreview.WorkspacesApi();
let body = new AsanaPreview.WorkspaceGidRemoveUserBody(); // WorkspaceGidRemoveUserBody | The user to remove from the workspace.
let workspace_gid = "12345"; // String | Globally unique identifier for the workspace or organization.

apiInstance.removeUserForWorkspace(body, workspace_gid, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
...
# **updateWorkspace**
> WorkspaceResponseData updateWorkspace(body, workspace_gid, opts)
